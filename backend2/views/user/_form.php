...
<div class="user-form">
    ...
    <?= $form->field($model, 'agama')->dropDownList(User::getAgama(),['prompt' => '-- Pilih Agama --']) ?>

    <?= $form->field($model, 'pict')->widget(FileInput::className(), ['options' => ['accept' => 'image/*',],'pluginOptions' => ['initialPreview' => [!$model->isNewRecord ? (!empty($model->pict) ? Html::img(\Yii::$app->cv->lihatImageDetail($model->pict, "", $kategori = "user")) : NULL) : null],'allowedFileExtensions' => ['jpg','jpeg'],'showUpload' => false,],'pluginEvents' =>[]]); ?>
